...
Public Class ctlProposta
    ...
    Public Function Pesquisar(ByVal oTipo As TipoPesquisaProposta, ByVal oParametros As Object) As DataTable
        ...
        Dim parmMatricula As New SqlParameter("@P_MATRICULA", DirectCast(HttpContext.Current.Session("Usuario2"), Dados.Usuario).UserCode) 'HttpContext.Current.User.Identity.Name)
    ...
    Public Function Selecionar(ByVal sNumero As String) As DataTable
        ...
        cmd.Parameters.Add(New SqlParameter("@P_MATRICULA", DirectCast(HttpContext.Current.Session("Usuario2"), Dados.Usuario).UserCode)) 'HttpContext.Current.User.Identity.Name))
        cmd.Parameters.Add(New SqlParameter("@P_FILIAL", Util.GetFilial()))
        Dim da As New SqlDataAdapter(cmd)
        Dim dt As New DataTable
        da.Fill(dt)
    ...
    Public Function Desativar(ByVal sNumero As String) As RetornoGenerico
        If Util.GetAmbiente().ToUpper = "DESENV" Then
            Dim oTokenD As New wshumberto.wsorcamento.TOKENSTRUCT
            oTokenD.CONTEUDO = HttpContext.Current.User.Identity.Name
            oTokenD.SENHA = DirectCast(HttpContext.Current.Session("Usuario2"), Dados.Usuario).Hash
            Dim wsOrcamentoD As New wshumberto.wsorcamento.WSORCAMENTO
            Dim oRetSigaD As wshumberto.wsorcamento.RETORNOSTRUCT = wsOrcamentoD.CANCELAR(oTokenD, sNumero)
            Return New RetornoGenerico(oRetSigaD)
        Else
            Dim oToken As New wshumberto.wsorcamento.TOKENSTRUCT
            ...
            Dim wsOrcamento As New wshumberto.wsorcamento.WSORCAMENTO
            Dim oRetSiga As wshumberto.wsorcamento.RETORNOSTRUCT = wsOrcamento.CANCELAR(oToken, sNumero)
            Return New RetornoGenerico(oRetSiga)
        End If
    End Function
    ...
    Public Function Alterar(ByVal oProposta As Proposta1.Proposta) As RetornoGenerico
        If Util.GetAmbiente().ToUpper = "DESENV" Then
            ...
            oTokenD.SENHA = DirectCast(HttpContext.Current.Session("Usuario2"), Dados.Usuario).Hash
            Dim wsOrcamentoD As New wshumberto.wsorcamento.WSORCAMENTO
            Dim oFichaD As New wshumberto.wsorcamento.FICHAORCAMENTOSTRUCT
            oFichaD = DirectCast(oProposta.PutSiga(oFichaD), wshumberto.wsorcamento.FICHAORCAMENTOSTRUCT)
            Dim oItensD As New List(Of wshumberto.wsorcamento.ITEMORCAMENTOSTRUCT)
            For Each oItemProposta As Proposta1.ItemProposta In oProposta.Itens
                Dim oItemD As New wshumberto.wsorcamento.ITEMORCAMENTOSTRUCT
                oItemD = DirectCast(oItemProposta.PutSiga(oItemD), wshumberto.wsorcamento.ITEMORCAMENTOSTRUCT)
                oItensD.Add(oItemD)
            Next
            oFichaD.ITENS = oItensD.ToArray
            Dim oRetSigaD As wshumberto.wsorcamento.RETORNOSTRUCT = wsOrcamentoD.ALTERAR(oTokenD, oFichaD)
            Return New RetornoGenerico(oRetSigaD)
        Else
            Dim oToken As New wshumberto.wsorcamento.TOKENSTRUCT
            oToken.CONTEUDO = HttpContext.Current.User.Identity.Name
            oToken.SENHA = DirectCast(HttpContext.Current.Session("Usuario2"), Dados.Usuario).Hash
            Dim wsOrcamento As New wshumberto.wsorcamento.WSORCAMENTO
            Dim oFicha As New wshumberto.wsorcamento.FICHAORCAMENTOSTRUCT
            oFicha = DirectCast(oProposta.PutSiga(oFicha), wshumberto.wsorcamento.FICHAORCAMENTOSTRUCT)
            Dim oItens As New List(Of wshumberto.wsorcamento.ITEMORCAMENTOSTRUCT)
            For Each oItemProposta As Proposta1.ItemProposta In oProposta.Itens
                Dim oItem As New wshumberto.wsorcamento.ITEMORCAMENTOSTRUCT
                oItem = DirectCast(oItemProposta.PutSiga(oItem), wshumberto.wsorcamento.ITEMORCAMENTOSTRUCT)
                oItens.Add(oItem)
            Next
            oFicha.ITENS = oItens.ToArray
            Dim oRetSiga As wshumberto.wsorcamento.RETORNOSTRUCT = wsOrcamento.GERAR(oToken, oFicha)
            Return New RetornoGenerico(oRetSiga)
        End If
    End Function
    Public Function Incluir(ByVal oProposta As Proposta1.Proposta) As RetornoGenerico
        If Util.GetAmbiente().ToUpper = "DESENV" Then
            ...
            Next
            oFichaD.ITENS = oItensD.ToArray
            Dim oRetSigaD As wshumberto.wsorcamento.RETORNOSTRUCT = wsOrcamentoD.GERAR(oTokenD, oFichaD)
            Return New RetornoGenerico(oRetSigaD)
        Else
            Dim oToken As New wshumberto.wsorcamento.TOKENSTRUCT
            oToken.CONTEUDO = HttpContext.Current.User.Identity.Name
    ...
    Public Function Aprovar(ByVal oProposta As Proposta1.Proposta) As RetornoGenerico
        If Util.GetAmbiente.ToUpper = "DESENV" Then
            ...
            Next
            oFichaD.ITENS = oItensD.ToArray
            Dim oRetSigaD As wshumberto.wsorcamento.RETORNOSTRUCT = wsOrcamentoD.APROVAR(oTokenD, oFichaD)
            Return New RetornoGenerico(oRetSigaD)
        Else
            Dim oToken As New wshumberto.wsorcamento.TOKENSTRUCT
            oToken.CONTEUDO = HttpContext.Current.User.Identity.Name
            oToken.SENHA = DirectCast(HttpContext.Current.Session("Usuario2"), Dados.Usuario).Hash
            Dim wsOrcamento As New wshumberto.wsorcamento.WSORCAMENTO
            'Dim oRetSiga As wshumberto.wsorcamento.RETORNOSTRUCT = wsOrcamento.APROVAR(oToken, oProposta.Numero)
            Return Nothing 'New RetornoGenerico(oRetSiga)
        End If
    End Function
